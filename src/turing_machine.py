    ...
    def process(self, verbose=False):
        ...
        while current_state.type != StateType.Final:
            ...
            try:
                ...
                transition_iter = iter(t for t in self.transitions 
                                  if t.current_state == state_id)
                
                transition = next(transition_iter)
        
            current_state = next(state for state in self.states if state.id == transition.new_state)

            step += 1

            if transition.new_char == '*' and current_state.type == StateType.Final:
                temp = 0 # do nothing, it reached the end state
            elif transition.new_char == '*' and current_state.type != StateType.Final:
